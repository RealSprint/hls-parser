...
function parseMasterPlaylist(lines, params) {
    ...
    params.playlistTransformers?.reduce((playlist, transformer) => transformer(playlist), playlist);
    return playlist;
}
function parseSegment(lines, uri, start, end, mediaSequenceNumber, discontinuitySequence, params) {
    const segment = new types_1.Segment({ uri, mediaSequenceNumber, discontinuitySequence, duration: -0 });
    let mapHint = false;
    let partHint = false;
...
function parseMediaPlaylist(lines, params) {
    ...
    for (const [index, line] of lines.entries()) {
        ...
        if (name === 'EXT-X-VERSION') {
            ...
            let segment = parseSegment(lines, line, segmentStart, index - 1, mediaSequence++, discontinuitySequence, params);
    ...
    if (segmentStart !== -1) {
        let segment = parseSegment(lines, '', segmentStart, lines.length - 1, mediaSequence++, discontinuitySequence, params);
        segment = transformSegment(segment, playlist, params);
        if (segment) {
            const { parts } = segment;
...
function transformSegment(segment, playlist, params) {
    const transformers = params.segmentTransformers;
    if (!transformers) {
        return segment;
    }
    const transformedSegment = transformers.reduce((currentSegment, transformer) => {
        if (!currentSegment) {
...
function parse(text, customParams = {}) {
    const defaultParams = {
        version: undefined,
