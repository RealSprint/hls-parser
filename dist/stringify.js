...
function buildSegment(lines, segment, lastKey, lastMap, version = 1) {
    ...
    Array.prototype.push.call(lines, `${segment.uri}`);
    ...
    for (const marker of markers) {
        if (marker.type === 'OUT' && !marker.adProviderSpecificTag) {
            ...
            if (marker.adProviderSpecificTag === 'transmit') {
                ...
                type = 'OUT';
                lines.push(`#EXT-X-TRANSMIT-CUE-OUT:AdFormat=${AdFormat},MaxDuration=${MaxDuration},KeepCreativeAudio=${KeepCreativeAudio},InsertionType=${InsertionType}`);
            }
        }
